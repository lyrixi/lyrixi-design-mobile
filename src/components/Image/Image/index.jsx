...
/* 测试使用-start
import { Bridge, LocaleUtil, Toast } from 'lyrixi-design-mobile'
测试使用-end */
  ...
  (
      ...
      [
        {
          thumb: "全路径(必传)",
          src: "全路径(必传)",
          path: "目录/年月/照片名.jpg(必传)",
          status: "choose|uploading|fail|success",
          "previewSrc|previewThumb": "本地预览路径, 用于离线场景(非必传)",
          children: node,
        },
      ]
      */
    ...
    async function handlePreview(item, index) {
      ...
      if (previewTypeRef.current === 'nativeImage') {
        Bridge.previewImage({
          urls: list.map((item) => item?.previewSrc || item.src),
          current: list[index]?.previewSrc || list[index].src,
          index: index
        })
      }
      // 视频使用previewFile预览
      else if (previewTypeRef.current === 'nativeFile') {
        Bridge.previewFile({ url: item.previewSrc || item.src })
      }
      // 浏览器预览
      else {
